...
export default function ChatBox({
  ...
  const sendMessage = (e: any) => {
    ...
    axios
      .post(
        `http://192.168.8.106:5555/chat/send?id=${conversation.id}`,
        { message: message },
  ...
  useEffect(() => {
    socket?.on("receiveMessage", (data) => {
      ...
      setIstyping(false);
      if (msg.chatId === conversation.id) setNewMessage(msg);
    });
    socket?.on("typing", (id) => {
      ...
      if (id === conversation.users[0].id) {
        ...
        setTimeout(() => {
          ...
          if (diff >= 1000) setIstyping(false);
        }, 1000);
      }
    });
  }, [socket]);
  ...
  return (
    <div className="bg-[#2a2a2e] basis-2/3 h-full z-0 justify-items-center">
      <div className="w-full h-[60px] bg-[#232327] shadow-xl italic text-white font-bold text-2xl fixed  p-3">
        <h1 className="ml-5">{conversation.users[0].userName}</h1>
      </div>
      <div className="h-[92%]  overflow-y-scroll flex flex-col-reverse p-6 scroll-smooth ">
        {isTyping ? <p className="text-white">typing...</p> : null}
        {allMessages?.map((msg) =>
          user?.id !== msg.userId ? (
            <div
              key={msg.id}
