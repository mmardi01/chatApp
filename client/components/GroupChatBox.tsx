  ...
  const socket = useContext(socketContext);
    ...
    useEffect(() => {
      ...
      const sendMessage = (e: any) => {
        ...
        if (message.length === 0)
          return 
      axios.post(`http://localhost:5555/group/send?id=${groupConversation.id}`,{message:message},{withCredentials:true})
      .then(res => {
        const msg : Message = res.data
        // socket?.emit('sendMessage',{message:msg,receiverId:groupConversation.id})
    ...
    <div className="w-full h-[60px] bg-[#232327] shadow-xl italic text-white font-bold text-2xl  p-3"><h1 className="ml-5">{groupConversation.name}</h1></div>
    <div className="h-[90%]  overflow-y-scroll flex flex-col-reverse p-6 scroll-smooth ">
      {allMessages?.map((msg) =>
        user?.id !== msg.userId ? (
          ...
          >
            <p className="break-all mr-2  max-w-[88%] text-[#555]">{msg.sender.userName}: </p>
